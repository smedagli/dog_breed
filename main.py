...
soa_network = 'InceptionV3'
for soa_network in ['VGG16', 'VGG19', 'Resnet50', 'InceptionV3', 'Xception']:
...
# # pred_train = model.predict(bottleneck_train)
#
# pred_test = model.predict(bottleneck_test)
# pred = np.array([np.argmax(x) for x in pred_test])
#
# metrics.get_accuracy(np.array([np.argmax(x) for x in pred_test]), np.array([np.argmax(y) for y in y_test]))
#
# (np.array([np.argmax(x) for x in pred_test]) == np.array([np.argmax(y) for y in y_test])).sum() / len(y_test) * 100
#
